...
class CocoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 🌟 修复原版危险的 _img_name[15:] 切片，改为更鲁棒的分割提取数字
        clean_name = str(_img_name).split('_')[-1]

        if self.stage == "train":
            label_dir = os.path.join(self.label_dir, clean_name + '.png')
            # 兜底：如果纯数字找不到，尝试原名
        ...
        return img_full_name, clean_name, image, label
