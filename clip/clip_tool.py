...
def perform_single_voc_cam(img_path, image, image_features, attn_weight_list, seg_attn, bg_text_features,
                           fg_text_features, cam, mode='train', require_seg_trans=False, dino_att=None,
                           seg_dino_cam=None, clip_flag=16):
    ...
    for idx, label in enumerate(label_list):
        ...
        if dino_att is not None:
            ...
            dino_att_weight = torch.mean(dino_att[-2:], dim=0)
            ...
            dino_cam_refine = F.interpolate(dino_cam_refine, size=(h // 16, w // 16), mode='bilinear',
                                            align_corners=False)
