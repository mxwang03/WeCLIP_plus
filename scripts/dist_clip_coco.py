...
parser.add_argument("--config",
                    default='/home/share/priv/wangmengxiang/code/WeCLIP_plus/configs/coco_attn_reg.yaml',
                    type=str,
                    help="config")
...
def get_seg_loss(pred, label, ignore_index=255):
    ...
    if torch.any(bg_label != ignore_index):
        losses.append(F.cross_entropy(pred, bg_label.type(torch.long), ignore_index=ignore_index))

    fg_label = label.clone()
    fg_label[label==0] = ignore_index
    if torch.any(fg_label != ignore_index):
        losses.append(F.cross_entropy(pred, fg_label.type(torch.long), ignore_index=ignore_index))
...
def train(cfg):
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        pred_prob = F.softmax(segs, dim=1)
        pred_max, pred_label_seg = torch.max(pred_prob, dim=1)
