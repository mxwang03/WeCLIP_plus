...
class CMCA_Module(nn.Module):
    ...
    def forward(self, clip_fts, dino_fts):
        ...
        semantic_mask = torch.sigmoid(semantic_sim / self.temp_semantic)
        ...
        dino_aff = F.softmax(dino_aff / self.temp_physical, dim=-1)
...
class WeCLIP_Plus(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, dino_model=None, dino_fts_dim=768, decoder_layers=3,
        ...
        self.bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY_COCO, prompt_templates, self.encoder)
        self.fg_text_features = zeroshot_classifier(new_class_names_coco, prompt_templates, self.encoder)
    ...
    def forward(self, img, img_names='2007_000032', mode='train'):
        ...
        # 根据 VOC 取消 self.training 限制，保留 enumerate 以自适应 batch
        for i, img_name in enumerate(img_names):
            # 保留 COCO 特有的路径结构 (.png, /train/)
